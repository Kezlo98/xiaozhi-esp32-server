...
# This script is used for one-click automatic download of project files and directory creation
# Currently only supports X86 Ubuntu systems, other systems not tested
...
echo -e "\e[1;36m  Xiaozhi Server Full Deployment One-Click Install Script Ver 0.2 Updated Aug 20, 2025 \e[0m\n"
sleep 1
...
if check_installed; then
    if whiptail --title "Installation Detected" --yesno "Xiaozhi Server is already installed. Do you want to upgrade?" 10 60; then
        # User chose to upgrade, execute cleanup
        echo "Starting upgrade..."

        # Stop and remove all docker-compose services
        docker compose -f /opt/xiaozhi-server/docker-compose_all.yml down

        # Stop and delete specific containers (handle case where container may not exist)
        containers=(
        ...
        # Delete specific images (handle case where image may not exist)
        images=(
        ...
        if [ -f /opt/xiaozhi-server/data/.config.yaml ]; then
            ...
            echo "Backed up original config to /opt/xiaozhi-server/backup/.config.yaml"
        fi
        ...
        # Mark upgrade completed, skip subsequent download steps
        UPGRADE_COMPLETED=1
        docker compose -f /opt/xiaozhi-server/docker-compose_all.yml up -d
          ...
          # Skip upgrade, continue with subsequent installation flow
    fi
fi
...
if ! command -v docker &> /dev/null; then
    ...
    # Use Chinese mirror instead of official source
    ...
    # Create keyring directory and add mirror GPG key
    ...
    # Add backup official source key (avoid mirror key verification failure)
...
MIRROR_CHOICE=$(whiptail --title "Select Docker Mirror" --menu "Please select the Docker mirror to use" 20 60 10 \
...
EOF
    whiptail --title "Configuration Successful" --msgbox "Successfully added mirror: $MIRROR_URL\nPress Enter to restart Docker service and continue..." 12 60
    echo "------------------------------------------------------------"
...
if [ ! -f "$MODEL_PATH" ]; then
    ...
    ) | whiptail --title "Downloading" --gauge "Downloading speech recognition model..." 10 60 0
...
# Only download if upgrade not completed
...
if [ $? -ne 0 ]; then
    whiptail --title "Error" --msgbox "Docker service startup failed. Please try changing mirror and run script again" 10 60
    exit 1
fi
...
whiptail --title "Configure Server Secret Key" --msgbox "Please use a browser to access the link below, open the Control Panel and register an account: \n\nLocal address: http://127.0.0.1:8002/\nPublic address: http://$PUBLIC_IP:8002/ (For cloud servers, please allow ports 8000 8001 8002 in security group).\n\nThe first registered user is the super admin. Subsequent users are regular users. Regular users can only bind devices and configure agents; super admin can manage models, users, and parameters.\n\nPress Enter to continue after registration" 18 70
SECRET_KEY=$(whiptail --title "Configure Server Secret Key" --inputbox "Login to Control Panel with super admin account\nLocal address: http://127.0.0.1:8002/\nPublic address: http://$PUBLIC_IP:8002/\nIn top menu: Parameter Dictionary -> Parameter Management, find parameter code: server.secret (Server Secret Key)\nCopy the value and enter below\n\nEnter secret key (leave empty to skip):" 15 60 3>&1 1>&2 2>&3)
...
# Fixed hardcoded addresses (previously had issues getting ws from logs)
